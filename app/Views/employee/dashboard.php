...
<body >
    ...
    <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
        ...
        <div class="body-wrapper">
                                    ...
                                    <div class="form-actions">
                                        ...
                                        <div
                                            ...
                                            <div class="d-flex gap-2">
                                                <a href="<?= base_url('/employee/prevRecord/') ?><?php echo $recordId; ?>"
            ...
            <a href="<?= base_url('/employee/forwardRecord/') ?><?php echo $recordId; ?>"
                                                    class="btn btn-warning rounded-circle round-40 btn-sm d-inline-flex align-items-center justify-content-center">
                            ...
                            if (uploadForm) {
                                uploadForm.addEventListener('submit', async function(event) {
                                    ...
                                    const feedback = document.getElementById(
                                        'uploadPolicyFeedback');
                                    if (submitBtn) {
                                        submitBtn.disabled = true;
                                        submitBtn.textContent = 'Uploading...';
                                    }
                                    if (feedback) {
                                        feedback.textContent = 'Uploading policy...';
                                    }

                                    const formData = new FormData(uploadForm);
                                    try {
                                        const response = await fetch(
                                            '<?= base_url('employee/upload-policy-ajax') ?>', {
                                                method: 'POST',
                                                body: formData
                                            });
                                        const result = await response.json();
                                        if (result.success) {
                                            ...
                                            const modal = bootstrap.Modal.getInstance(document
                                                .getElementById('uploadPolicyModal'));
                                            if (modal) {
                                                modal.hide();
                                            }
                                        } else {
                                        ...
                                        showMessage('danger',
                                            'Network error while uploading policy.');
                                        if (feedback) {
                                            feedback.textContent =
                                                'Network error while uploading policy.';
                                        }
